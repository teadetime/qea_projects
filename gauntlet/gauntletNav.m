...
y = [];
syms x y
% Use global variables to simplify the field manipulation
...
%Parameters to visualize whether to see plots
view_fields = 0
view_ransac = 0
theta_change = [];
distance_change = [];
...
while 1
    ...
    % the neato now
    %THIS RUNS IN RELATIVE MODE (MULTIPLE SCANS AND TURNS DONE RELATIVE TO THE NEATO"S POSITION)
    %THIS DOENS"T KEEP TRACK OF IT"S POSITION AND IS USED TO DRIVE THE ROPBOT
    process_scan(x, y, scan_r, scan_theta, neato_x, neato_y, neato_phi, d, 1, view_ransac);
    %SWITCH TO THE BELOW LINE TO GENERATE PLOTS IN GLOBAL FRAME **THIS WILL
    %NOT LET THE ROBOT DRIVE CORRECTLY
    %process_scan(x, y, scan_r, scan_theta, neato_x, neato_y, neato_phi, d, 0, view_ransac);
    
    % Now calculate the gradient and 
    sym_grad = gradient(v_symbolic)
    
    % Determine steepest descent and the angle which that is
    ...
    % Also manually makes a plot with 8 points to the BOB
    if view_fields == 1
        ...
        %UNCOMMENT IF RUNNING IN RELATIVE FRAME
        %plot(r(:,1),r(:,2), '--r')
        hold on
        ...
        % PLot awesome mesh in 3d for descent visualization
        figure 
